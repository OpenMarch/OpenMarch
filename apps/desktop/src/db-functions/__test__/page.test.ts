...
import { createPages, updatePages, deletePages, FIRST_PAGE_ID } from "../page";
import { describeDbTests, schema } from "@/test/base";
import { getTestWithHistory } from "@/test/history";
import { inArray } from "drizzle-orm";
import { getMarchers } from "../marcher";
            ...
            describe("insert with existing pages", () => {
                testWithHistory.for([
                    ...
                    async (
                        ...
                        expect(
                            createdExistingPages.sort(sortByBeat),
                        ).toMatchObject(existingPagesArgs.sort(sortByBeat));
                ...
                describe("insert with existing pages", () => {
                    testWithHistory.for([
                        ...
                        async (
                            ...
                            await expectNumberOfChanges.test(2); // One for existing pages, one for new pages
                        },
                    );
                });
            });
                ...
                describe("deletePage with failure", () => {
                    testWithHistory(
                        ...
                        async ({
                            ...
                            expect(pagesAfterDelete[0].id).toEqual(
                                firstPage.id,
                            );
                        },
                    );
                    ...
                    testWithHistory.for([
                        ...
                        // },
                        {
                            ...
                            fakePageIdsToDelete: [
                                7987, 8273623, -1, 123456, 986, 6275.2378,
                                -128.2,
                            ],
                        },
                    ])(
                    ...
                    testWithHistory(
                        ...
                        async ({
                            ...
                            expect(marcherPagesAfterDelete.length).toBe(
                                marchersAndPages.expectedMarcherPages.length -
                                    existingPageIndexesToDelete.length *
                                        marchersAndPages.expectedMarchers
                                            .length,
                            );
                            await expectNumberOfChanges.test(
