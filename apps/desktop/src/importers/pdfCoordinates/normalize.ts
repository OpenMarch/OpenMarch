...
const LATERAL_REGEX =
    /(?:On|on)|(?:(\d+(?:\.\d+)?)\s*steps\s*(Inside|Outside)\s*(\d{1,2})\s*yd\s*ln)/i;
const FB_REGEX =
    /^(?:On|on)\s*(Front|Back)\s*Hash|^(\d+(?:\.\d+)?)\s*steps\s*(In Front Of|Behind)\s*(Front|Back)\s*Hash/i;
...
function getYCheckpoint(
    ...
    if (!preferred)
        throw new Error(`No ${which} hash found in field properties`);
    return preferred;
}
