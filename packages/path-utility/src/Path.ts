import { IPath, IPathSegment, Point, PathJsonData, SegmentJsonData } from './interfaces';
...
export class Path implements IPath {
    ...
    getTotalLength(): number {
        return this._segments.reduce((total, segment) => total + segment.getLength(), 0);
    }
    ...
    fromJson(json: string): IPath {
        try {
            ...
            if (!pathData.segments || !Array.isArray(pathData.segments)) {
                throw new Error('Invalid path JSON: missing or invalid segments array');
            }

            const segments: IPathSegment[] = pathData.segments.map(segmentData => {
                return this.createSegmentFromJson(segmentData);
            });

            return new Path(segments);
        } catch (error) {
            throw new Error(`Failed to parse path JSON: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
            ...
            switch (type.toLowerCase()) {
                ...
                case 'c': // Cubic curve
                    if (args.length >= 6) {
                        ...
                        const cp2 = type === 'C'
                            ? { x: args[2], y: args[3] }
                            : { x: currentPoint.x + args[2], y: currentPoint.y + args[3] };
                        const end = type === 'C'
                            ? { x: args[4], y: args[5] }
                            : { x: currentPoint.x + args[4], y: currentPoint.y + args[5] };
                        segments.push(new CubicCurve(currentPoint, cp1, cp2, end));
                        currentPoint = end;
                    }
                    break;
                ...
                case 'a': // Arc - simplified handling
                    if (args.length >= 7) {
                        const endPoint = type === 'A'
                            ? { x: args[5], y: args[6] }
                            : { x: currentPoint.x + args[5], y: currentPoint.y + args[6] };
                        
                        // Simplified arc creation - for production, properly parse all arc parameters
                        const center = {
                            x: (currentPoint.x + endPoint.x) / 2,
                            y: (currentPoint.y + endPoint.y) / 2
                        };
                        const radius = args[0]; // rx
                        const startAngle = Math.atan2(currentPoint.y - center.y, currentPoint.x - center.x);
                        const endAngle = Math.atan2(endPoint.y - center.y, endPoint.x - center.x);
                        
                        segments.push(new Arc(center, radius, startAngle, endAngle, args[4] === 1));
                        currentPoint = endPoint;
                    }
                    break;
