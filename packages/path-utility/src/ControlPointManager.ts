...
export class ControlPointManager {
    ...
    getControlPointsForSegment(segmentIndex: number): ControlPoint[] {
        return this.getAllControlPoints().filter(cp => cp.segmentIndex === segmentIndex);
    }
    ...
    moveControlPoints(updates: Array<{ id: string; point: Point }>): boolean {
        ...
        for (const update of updates) {
            ...
            const segment = this._path.segments[controlPoint.segmentIndex];
            if (!segment || !this.isControllableSegment(segment)) {
                console.warn(`Segment ${controlPoint.segmentIndex} is not controllable`);
                continue;
            }
        ...
        try {
            ...
            for (const [segmentIndex, originalSegment] of segmentsToUpdate) {
                ...
                for (const update of updates) {
                    ...
                    if (controlPoint && controlPoint.segmentIndex === segmentIndex) {
     ...
     */
    getControlPointAt(point: Point, tolerance: number = 10): ControlPoint | null {
    ...
    getControlPointsNear(point: Point, tolerance: number = 10): ControlPoint[] {
        ...
        return nearby.sort((a, b) => 
            this.getDistance(point, a.point) - this.getDistance(point, b.point)
        );
    }

    private isControllableSegment(segment: IPathSegment): segment is IControllableSegment {
    ...
    private generateControlPointId(segmentIndex: number, type: ControlPointType, pointIndex?: number): string {
