...
- Two chunk families are exceptions to the length rule (see below): the page
  ...
  (`SEL2`/`VIS2`). `VIS2`'s declared length has been observed wrong on a real
  file (undercounting the actual payload by several KB), so it is treated as
  advisory, not trusted blindly — see the `VIS2` row below and the resync
  logic in `skipChunk` (`document.ts`).
...
| `VIS2` | —     | 13    | View snapshots — `u16 count` then full 69-byte marker records (§2.6). Editor state. **Its declared length has been observed wrong on a real file** (undercounting the real payload by ~8.6 KB) — the reader verifies where it lands and resyncs by scanning forward for the next recognizable chunk when it doesn't land cleanly (`skipChunk`, `document.ts`), rather than trusting the declared length as-is. |
