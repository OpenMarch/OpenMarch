...
const headerNeedles = {
    ...
    fbText: [/^front to back$/, /^front\s*[-/]?\s*to\s*back$/, /^front back$/],
};
...
export function detectHeaderAndBands(
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        // Require at least 3 header labels to trust this as header
        ...
        if (uniqueKeys.length >= 3) {
            ...
            // Sort by x and apply fallback for last two columns:
            // if lateralText/fbText are missing or mislabeled, treat the two rightmost bands as X then Y
