...
### 0. **Figure out if you're only creating 'inTransaction' functions**

In a table management file, we typically have two types of database functions for `create`, `update`, and `delete`:

1. `{action}TableItemInTransaction` - (e.g. `createItemInTransaction`)
   1. Accepts a `DbTransaction` as a `tx` parameter
   1. Where the core logic of the query is
1. `{action}TableItem` - (e.g. `createItem`)
   1. Accepts a `DbConnection` as a `db` parameter
   1. Doesn't have much logic in it
   1. Wraps the `...InTransaction` equivalent around a `transactionWithHistory`
   1. Standalone function that uses its own history group

These are separate to cater to different needs. If we provided the non-`InTransaction` function, we are expecting that function to be called a lot on its own.

This is getting rarer and rarer as we often need database functions to be part of a larger transaction.

Because of this **by default, you should only provide the `{action}InTransaction` functions, not the standalone ones.** Only deviate from this if the user explicitly asks. The examples below are just for you to use for reference in case they ask for it.
 ...
 */
// ONLY PROVIDE STANDALONE FUNCTION IF USER ASKS FOR IT
export async function create{tableName}s({
    ...
    if (newItems.length === 0) {
        console.warn("No new items to create");
        return [];
    }
 ...
 * Creates new items in the database.
 */
export const create{tableName}sInTransaction = async ({
    newItems,
    tx,
}: {
    newItems: New{tableName}Args[];
 ...
 */
export const delete{tableName}sInTransaction = async ({
    itemIds,
    tx,
}: {
    itemIds: Set<number>;
...
- ✅ Put all of the core database logic in a "inTransaction" function that takes a transaction as an argument
