...
CREATE TABLE `__new_marcher_pages` (
	...
	CONSTRAINT "marcher_pages_path_data_position_check" CHECK(svg_position >= 0 AND svg_position <= 1)
);
--> statement-breakpoint
INSERT INTO `__new_marcher_pages`("id", "id_for_html", "marcher_id", "page_id", "x", "y", "created_at", "updated_at", "path_data_id", "svg_position", "notes") SELECT "id", "id_for_html", "marcher_id", "page_id", "x", "y", "created_at", "updated_at", "svg_path", "svg_position", "notes" FROM `marcher_pages`;--> statement-breakpoint
...
CREATE TABLE `__new_midsets` (
	...
	FOREIGN KEY (`page_id`) REFERENCES `pages`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`path_data_id`) REFERENCES `pathways`(`id`) ON UPDATE no action ON DELETE set null,
	CONSTRAINT "midsets_path_data_position_check" CHECK(svg_position >= 0 AND svg_position <= 1),
	CONSTRAINT "placement_check" CHECK(placement > 0 AND placement < 1)
);
--> statement-breakpoint
INSERT INTO `__new_midsets`("id", "page_id", "x", "y", "placement", "created_at", "updated_at", "path_data_id", "svg_position", "notes") SELECT "id", "page_id", "x", "y", "placement", "created_at", "updated_at", "svg_path", "svg_position", "notes" FROM `midsets`;--> statement-breakpoint
