...
export class Spline implements IPathSegment {
    ...
    fromJson(data: SegmentJsonData): IPathSegment {
        if (data.type !== 'spline') {
            throw new Error(`Cannot create Spline from data of type ${data.type}`);
        }
        return new Spline(
    ...
    static fromJson(data: SegmentJsonData): Spline {
        const instance = new Spline([{ x: 0, y: 0 }, { x: 1, y: 1 }]);
     ...
     */
    static fromPoints(points: Point[], tension: number = 0.5, closed: boolean = false): Spline {
     ...
     */
    static createBSpline(controlPoints: Point[], degree: number, knots: number[], weights?: number[]): Spline {
