UPDATE `lighting_effects` SET `type` = 'solid' WHERE `type` = 'strobe';--> statement-breakpoint
UPDATE `lighting_effects` SET `type` = 'solid', `args` = json_object('color', coalesce(json_extract(`args`, '$.colors[0]'), json_extract(`args`, '$.color'), '#000000')) WHERE `type` = 'fade';--> statement-breakpoint
...
CREATE TABLE `__new_lighting_effects` (
	...
	CONSTRAINT "lighting_effect_type_check" CHECK("__new_lighting_effects"."type" IN ('solid', 'wipe', 'flicker')),
