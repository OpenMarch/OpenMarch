import { sqliteTable, AnySQLiteColumn, check, integer, text, real, foreignKey, index, blob } from "drizzle-orm/sqlite-core"
...
(table) => [
	check("history_stats_check_1", sql`id = 1`),
	check("beats_check_2", sql`duration >= 0`),
	check("beats_check_3", sql`position >= 0`),
	check("beats_check_4", sql`include_in_measure IN (0, 1`),
	check("pages_check_5", sql`is_subset IN (0, 1`),
	check("field_properties_check_6", sql`id = 1`),
...
export const measures = sqliteTable("measures", {
	id: integer().primaryKey(),
	start_beat: integer().notNull().references(() => beats.id),
	rehearsal_mark: text(),
	notes: text(),
	created_at: text().default("sql`(CURRENT_TIMESTAMP)`").notNull(),
	updated_at: text().default("sql`(CURRENT_TIMESTAMP)`").notNull(),
...
export const marcher_pages = sqliteTable("marcher_pages", {
	id: integer().primaryKey(),
	id_for_html: text(),
	marcher_id: integer().notNull().references(() => marchers.id, { onDelete: "cascade" } ),
	page_id: integer().notNull().references(() => pages.id, { onDelete: "cascade" } ),
	x: real(),
	y: real(),
	created_at: text().notNull(),
	updated_at: text().notNull(),
...
export const shape_pages = sqliteTable("shape_pages", {
	id: integer().primaryKey(),
	shape_id: integer().notNull().references(() => shapes.id, { onDelete: "cascade" } ),
	page_id: integer().notNull().references(() => pages.id, { onDelete: "cascade" } ),
...
export const shape_page_marchers = sqliteTable("shape_page_marchers", {
	id: integer().primaryKey(),
	shape_page_id: integer().notNull().references(() => shape_pages.id),
	marcher_id: integer().notNull().references(() => marchers.id),
