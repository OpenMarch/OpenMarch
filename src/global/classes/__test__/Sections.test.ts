...
describe('Section', () => {
    ...
    describe('compareTo', () => {
        it('should return a negative number if the scoreOrder of the first section is greater than the second section', () => {
            expect(SECTIONS.Snare.compareTo(SECTIONS.Trumpet)).toBeGreaterThan(0);
            expect(SECTIONS.Trumpet.compareTo(SECTIONS.Piccolo)).toBeGreaterThan(0);
            expect(SECTIONS.Tuba.compareTo(SECTIONS.Trumpet)).toBeGreaterThan(0);
            // Even though DrumMajor is a higher scoreOrder than Marimba, the "Other" family is higher than "Pit"
            expect(SECTIONS.Marimba.compareTo(SECTIONS.DrumMajor)).toBeGreaterThan(0);
        });
