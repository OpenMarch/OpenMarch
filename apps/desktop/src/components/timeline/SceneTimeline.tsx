...
export default function SceneTimeline() {
    ...
    const [pendingSplitPageIndex, setPendingSplitPageIndex] = useState<
        number | null
    >(null);
    ...
    const splitAtPageIndex = useCallback(
        (pageIndex: number) => {
            ...
            setPendingSplitPageIndex(pageIndex);
        },
        [pages, scenes],
    );

    const createSceneAtPendingSplit = useCallback(() => {
        if (pendingSplitPageIndex == null) return;
        const page = pages[pendingSplitPageIndex];
        if (!page) return;
        if (scenes.some((s) => s.start_page_id === page.id)) return;

        setPendingSplitPageIndex(null);
        createScenes([{ start_page_id: page.id, name: null }]);
    }, [createScenes, pages, pendingSplitPageIndex, scenes]);

    const closeCreateSceneDialog = useCallback(() => {
        setPendingSplitPageIndex(null);
    }, []);

    const handleCreateSceneDialogOpenChange = useCallback(
        (isOpen: boolean) => {
            if (!isOpen) closeCreateSceneDialog();
        },
        [closeCreateSceneDialog],
    );

    const createSceneDialogOpen = pendingSplitPageIndex != null;
    ...
    return (
        <>
            <AlertDialog
                open={createSceneDialogOpen}
                onOpenChange={handleCreateSceneDialogOpenChange}
            >
                <AlertDialogContent>
                    <AlertDialogTitle>
                        <T keyName="timeline.createSceneTitle" />
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                        <T keyName="timeline.createSceneDescription" />
                    </AlertDialogDescription>
                    <div className="flex justify-end gap-8 pt-16">
                        <AlertDialogCancel asChild>
                            <Button variant="secondary" size="compact">
                                <T keyName="timeline.createSceneCancel" />
                            </Button>
                        </AlertDialogCancel>
                        <AlertDialogAction>
                            <Button
                                variant="primary"
                                size="compact"
                                onClick={createSceneAtPendingSplit}
                            >
                                <T keyName="timeline.createSceneConfirm" />
                            </Button>
                        </AlertDialogAction>
                    </div>
                </AlertDialogContent>
