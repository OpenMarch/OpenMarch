...
describe("Measure", () => {
    ...
    it("should create a measure object with correct duration for 6/16 time signature and dotted sixteenth beat unit", () => {
        const measure = new Measure({
            ...
            rehearsalMark: "A",
            tempo: 120,
            beatUnit: BeatUnit.DOTTED_SIXTEENTH,
...
G z z2  | z4 |  %2
    ...
    describe("insertMeasure", () => {
        describe("no existing measures", () => {
            it("should insert a single measure when there are none", async () => {
                window.electron = {
                    updateMeasureAbcString: vi
                        .fn()
                        .mockResolvedValue({ success: true }),
                    getMeasuresAbcString: vi.fn().mockResolvedValue(""),
                } as Partial<ElectronApi> as ElectronApi;
                // Spies
                const checkForFetchMeasuresSpy = vi.spyOn(
                    Measure,
                    "checkForFetchMeasures",
                );
                const fetchMeasuresSpy = vi.spyOn(Measure, "fetchMeasures");
                const electronUpdateMeasuresSpy = vi.spyOn(
                    window.electron,
                    "updateMeasureAbcString",
                );
                ...
                let response = await Measure.insertMeasure({
                    newMeasure,
                    existingMeasures: [],
                });
                ...
                expect(electronUpdateMeasuresSpy).toHaveBeenCalledWith(
                    expectedAbcString,
                );
                ...
                response = await Measure.insertMeasure({
                    newMeasure,
                    existingMeasures: [],
                });
                ...
                for (const measureData of measures) {
                    window.electron = {
                        updateMeasureAbcString: vi
                            .fn()
                            .mockResolvedValue({ success: true }),
                        getMeasuresAbcString: vi
                            .fn()
                            .mockResolvedValue(expectedAbcString),
                    } as Partial<ElectronApi> as ElectronApi;
                    // Spies
                    const checkForFetchMeasuresSpy = vi.spyOn(
                        Measure,
                    ...
                    const response = await Measure.insertMeasure({
                        newMeasure: measureData.measure,
                        existingMeasures,
                    });
        ...
        describe("existing measures", () => {
            it("should insert a single measure at the end when there are existing measures", async () => {
                ...
                window.electron = {
                    ...
                    getMeasuresAbcString: vi
                        .fn()
                        .mockResolvedValue(existingMeasuresAbcString),
                } as Partial<ElectronApi> as ElectronApi;
                // Spies
                const checkForFetchMeasuresSpy = vi.spyOn(
                    Measure,
            ...
            it("should insert a measure when the existing measures are provided", async () => {
                ...
                let existingMeasuresAbcString =
                    "X:1\nM:4/4\nQ:1/4=120\nV:1 baritone\nV:1\nz4 | z4 | z4 | ";
                const existingMeasures = Measure.abcToMeasures(
                    existingMeasuresAbcString,
                );
    ...
    describe("updateMeasure", () => {
        it("should update a single measure at the end when there are existing measures", async () => {
            ...
            const existingMeasuresAbcString =
                "X:1\nM:4/4\nQ:1/4=120\nV:1 baritone\nV:1\nz4 | z4 | z4 | ";
        ...
        describe("single measure at the start", () => {
            it("should update a single measure's tempo at the start ", async () => {
                ...
                const response = await Measure.updateMeasure({
                    modifiedMeasure,
                });
    ...
    describe("deleteMeasure", () => {
        ...
        describe("existing measures", () => {
            it("should delete a single measure at the end when there are existing measures", async () => {
                ...
                const response = await Measure.deleteMeasure({
                    measureNumber: 3,
                });
